...
var app = {
    ...
    onDeviceReady: function() {
        ...
        document.addEventListener("backbutton", function(e){
            ...
            var isMain = $$('#main').attr('class') == 'show';
            if(isMain){
...
function loadResource(){
    ...
    $$.ajax({
        ...
        success: function(data) {
            ...
            //check new version
            if(data.newVersion.version != '' && data.newVersion.version != zakatVersion){
                Lungo.Notification.confirm({
                    icon: 'cloud-download',
                    title: '新版本下载',
                    description: data.newVersion.description,
                    accept: {
                        label: '下载',
                        callback: function(){ window.open(data.newVersion.url, '_blank', 'closebuttoncaption=关闭,toolbar=yes'); }
                    },
                    cancel: {
                        label: '取消',
                        callback: function(){  }
